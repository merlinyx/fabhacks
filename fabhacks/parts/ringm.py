...
class RingM(Part):
    def __init__(self):
        super().__init__()
        self.tid = PartType.RingM
        self.name_in_assembly = self.__class__.__name__ + "_" + str(self.id)
        self.part_obj_path = os.path.join(".", Part.temp_dir, "RingM.obj")
        self.ring = Ring({"arc_radius": 53.3, "thickness": 3.3}, parent_part=self)
        self.ring.set_frame(Frame([0,0,0], [0,90,0]))
        self.ring.child_id = 0
        self.ring.set_ohe(PartPrimitiveType.RingOfRingM)
