...
from .mate_connector_frame import RodMCF, RodCtlMCF
from solid import *
import igl
import subprocess
import os
...
class Rod(Primitive):
    ...
    def init_params(self, params):
        ...
        if "centerline" in params and params["centerline"]:
            self.mcf_factory = RodCtlMCF(self.id, self.radius, self.length)
            self.ctype_str += "Ctl"
        else:
            self.mcf_factory = RodMCF(self.id, self.radius, self.length)
